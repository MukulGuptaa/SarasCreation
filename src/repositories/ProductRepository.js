...
class ProductRepository{
    ...
    async getAllProducts(page = 1, limit = 10, filters = [], loom = [], occassion = []) {
      try {
        ...
        // ------------------ LOOM FILTER (NEW, SEPARATE PARAM) ------------------
        if (Array.isArray(loom) && loom.length > 0) {
          filterQuery.loom = { $in: loom.map(l => l.trim()) };
        }

        // ------------------ OCCASSION FILTER (NEW, SEPARATE PARAM) ------------------
        if (Array.isArray(occassion) && occassion.length > 0) {
          filterQuery.occassion = { $in: occassion.map(o => o.trim()) };
        }
